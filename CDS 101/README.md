...
All groups will be working with the [College Scorecard](https://collegescorecard.ed.gov/data/) dataset started by The Obama Administration in September 2015.

The dataset is quite large (13.5 MB), and so cannot be distributed via GitHub. **You need to download the dataset and upload it** to RStudio's folder for this project. Once you have done so, it will loaded by default in the setup block of the `final_project.Rmd` file where you will write your group report (the variable name of the dataset is `college`).

Because the dataset is so large, you will find it almost impossible to open in a tab in RStudio. Please do not try to do so - your RStudio will freeze up. Instead, you will have to rely on a table describing all the columns in the dataset. This is called a *data dictionary*, and is available as an Excel spreadsheet at <https://collegescorecard.ed.gov/assets/CollegeScorecardDataDictionary.xlsx>, or as a Google Docs spreadsheet here: <https://docs.google.com/spreadsheets/d/1YrYiJ-J9rLhs4Qf5OCvHGMyUFyMKOMtMuGmpyt47mEY/edit#gid=532275090>.
...
If you are desperate to take a look at the raw data in table format, you could try saving just the first few rows as a new variable, and opening that in a tab in RStudio, e.g.:

```r
college_first6rows <- head(college)
```

The dataset includes data on a huge range of issues, including:
...
### Question of Interest section

In this section, you should write a paragraph or so that introduces the question that you are investigating, and explains why it is an interesting question. You also need to state what variables you will use to answer the question, and whether you will use a hypothesis test (inference) or modeling.
...
3.  Categorical variables that are not easy to understand, for example the integer categories under the `REGION` column, should be relabeled using the `recode` function.
...
You should calculate the summary statistics for each of the main variables that you will be including in your *Data Analysis* section. Most people will have two variables, if they followed the question structure suggested in the *Question of Interest* instructions above.
...
* Write out the null *and* alternative hypotheses at the start of this section (in text, not code).

* State whether you are performing a one-sided or two-sided test.
...
* **Question of Interest section**
  ...
  * State the columns you will be using to answer this question, and explain what data each variable contains.
...
* **Summary Statistics section**
  * Summary stats calculated for each of the variables identified in the Question of Interest section.
* **Data Analysis section**
...
Grades for the final project will be based on the correctness and readability of your R code, how well your report is written (the report should be structured, coherent, and follow the standard rules of spelling and grammar), and how well you answer your question of interest using your visualizations, analysis, and written answers.
